...
IV_PREP_REVISION <-
  "2026-08-09-v1.0.1-shric-zero-hotfix"
...
# ============================================================
# 2A. STRUCTURAL-ZERO AUDIT FOR SHRIC CELLS
# ============================================================
#
# In the SHRUG EC05 AC08 file, some individual ec05_emp_shric_k cells
# are Stata missing even though ec05_emp_all is observed. Before treating
# those cells as zero, require that the sum of ALL observed SHRIC cells
# exactly reproduces ec05_emp_all for every row containing such a missing
# SHRIC value. If that accounting identity fails, stop rather than impute.

sector_emp_matrix_raw <- as.matrix(
  dplyr::select(
    ec05,
    dplyr::all_of(
      sector_emp_vars
...
ec05$n_missing_shric_emp <-
  rowSums(
    is.na(
      sector_emp_matrix_raw
    )
  )

ec05$sum_observed_shric_emp <-
  rowSums(
    sector_emp_matrix_raw,
    na.rm = TRUE
  )

ec05$shric_missing_accounting_error <-
  ec05$sum_observed_shric_emp -
  as.numeric(
    ec05$ec05_emp_all
  )

structural_zero_audit <- ec05 |>
  dplyr::summarise(
    n_ec05_rows =
      dplyr::n(),

    n_rows_with_any_missing_shric =
      sum(
        n_missing_shric_emp > 0
      ),

    max_missing_shric_cells_in_row =
      max(
        n_missing_shric_emp
      ),

    max_abs_accounting_error_missing_rows =
      max(
        abs(
          shric_missing_accounting_error[
            n_missing_shric_emp > 0
          ]
        ),
        na.rm = TRUE
    ...
    all_missing_rows_account_exactly =
      all(
        abs(
          shric_missing_accounting_error[
            n_missing_shric_emp > 0
...
readr::write_csv(
  ...
  file.path(
    out_manifest_dir,
    "00_ec05_shric_structural_zero_audit.csv"
  )
)

if (
  !structural_zero_audit$all_missing_rows_account_exactly
) {
  stop(
    "At least one EC05 row with a missing SHRIC cell does not have ",
    "observed SHRIC employment summing to ec05_emp_all. ",
    "Do not convert these missing cells to zero."
  )
}

# The accounting identity is exact on every affected row, so the missing
# sector cells are structural zeros for purposes of the 90-industry shares.
ec05 <- ec05 |>
  dplyr::mutate(
    dplyr::across(
      dplyr::all_of(
...
readr::write_lines(
  c(
    ...
    "--------------------",
    "Some SHRUG EC05 SHRIC cells are stored as missing even when total employment is observed.",
    "The script converts them to zero ONLY after verifying, row by row, that the sum of all observed",
    "SHRIC employment exactly equals ec05_emp_all for every affected AC. The audit is written to",
    "manifests/00_ec05_shric_structural_zero_audit.csv.",
    "",
