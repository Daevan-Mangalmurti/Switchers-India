...
# ------------------------------------------------------------
# OPERATIONAL ROBUSTNESS SHARD SETTINGS
# ------------------------------------------------------------
# This runner changes execution only, NOT the frozen model specification or
# RESPONDENT_SCRIPT_REVISION. Per-curve filenames therefore remain compatible
# with the v1.0.3 cache/checkpoint scheme.
if (
  "primary" %in%
    RUN_ANALYSIS_FAMILIES
) {
  stop(
    "This operational runner is for robustness families only. ",
    "Run the frozen primary universe with the original v1.0.3 script."
  )
}

ROBUSTNESS_DESIGN_FILTER <- trimws(
  Sys.getenv(
    "SWITCHERS_RESPONDENT_DESIGN_FILTER",
    unset = ""
  )
)
...
if (
  nzchar(
    ROBUSTNESS_DESIGN_FILTER
  )
) {
  requested_designs <- trimws(
    unlist(
      strsplit(
        ROBUSTNESS_DESIGN_FILTER,
        ",",
  ...
  unknown_designs <- setdiff(
    requested_designs,
    respondent_design_meta$design_id
  )
  ...
  if (
    ...
    stop(
      "Unknown respondent design filter(s): ",
      paste(
        unknown_designs,
        collapse = ", "
      )
  ...
  respondent_design_meta <- respondent_design_meta |>
    dplyr::filter(
      design_id %in%
        requested_designs
    )
...
if (
  ...
  stop(
    "Operational robustness design filter left zero respondent designs."
  )
}

ROBUSTNESS_RUNNER_TAG <- paste(
  c(
    paste(
      RUN_ANALYSIS_FAMILIES,
      collapse = "-"
...
message(
  "Operational robustness runner active."
)
message(
  "  Families: ",
  paste(
...
message(
  "  Aggregate-output tag: ",
  ROBUSTNESS_RUNNER_TAG
)
...
# ============================================================
# 18. OPERATIONAL ROBUSTNESS RUNNER EXIT
# ============================================================

message("")
message(
  "Robustness shard COMPLETE: ",
  ROBUSTNESS_RUNNER_TAG
)
message(
  "Per-curve results: ",
...
message(
  "Exiting before primary figures/preferred-model sections so robustness-only ",
  "jobs cannot overwrite the frozen primary preferred outputs."
)
...
stop(
  "Robustness shard finished successfully. ",
  "Run this operational script via Rscript from Terminal."
)
