...
WIR_SHOCK_REVISION <-
  "2026-08-09-v1.0.2-share-completeness-provenance"
...
fdi_provenance_audit <- tibble::tibble(
  source_path =
    fdi_path,
  ...
  n_missing_project_type =
    sum(
      is.na(
        fdi_raw$`Project type`
      ) |
      trimws(
        as.character(
          fdi_raw$`Project type`
  ...
  n_missing_project_status =
    sum(
      is.na(
        fdi_raw$`Project status`
      ) |
      trimws(
        as.character(
          fdi_raw$`Project status`
...
readr::write_csv(
  fdi_provenance_audit,
  file.path(
    out_manifest_dir,
    "02a_india_fdi_source_provenance.csv"
  )
)
...
share_vars_iv <- paste0(
  "ec05_share_shric_",
  1:90
)

ec05_share_completeness_audit <- iv_base |>
  dplyr::filter(
    !is.na(
      ec05_emp_all
    )
  ...
  dplyr::summarise(
    ...
    n_rows_with_any_missing_shric_share =
      sum(
        !dplyr::if_all(
          dplyr::all_of(
            share_vars_iv
...
readr::write_csv(
  ec05_share_completeness_audit,
  file.path(
    out_manifest_dir,
    "00_ec05_share_completeness_for_bartik.csv"
  )
)

if (
  ec05_share_completeness_audit$n_rows_with_any_missing_shric_share >
    0
) {
  stop(
    "EC05 rows still contain missing SHRIC employment shares. ",
    "Rerun corrected 06_iv_design_prep_ec05_v1_0_1.R before 07."
  )
}
...
readr::write_lines(
  c(
    ...
    "manifests/02a_india_fdi_source_provenance.csv",
