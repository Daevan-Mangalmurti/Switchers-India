...
# ------------------------------------------------------------
# OPERATIONAL RESUME SHARD
# ------------------------------------------------------------
# This is an execution-only shard. It does NOT change the frozen model
# specification or RESPONDENT_SCRIPT_REVISION. Restrict the run to the only
# unfinished design from the aborted full-primary run so completed curves are
# not loaded into memory.
respondent_design_meta <- respondent_design_meta |>
  dplyr::filter(
    design_id == "respondent_2014_migration"
  )

message(
  "Operational resume shard active: respondent_2014_migration only."
)
...
message("")
message("2014 migration resume shard complete.")
message("Per-curve result files have been written to: ", respondent_result_dir)
message("Aggregate shard summaries have been written to: ", respondent_summary_dir)
message("Exiting cleanly before figure/targeted-output stages to minimize memory use.")
...
stop(
  "Resume shard finished successfully. This script is intended to be run with Rscript from Terminal; no further estimation is required in this process."
)
