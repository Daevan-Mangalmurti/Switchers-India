...
CANDIDATE_SUPPLY_REVISION <-
  "2026-08-08-v1.0.2-bjp-flag-reconciliation"
...
# A mismatch here can arise from the raw PC-to-analysis-PC mapping rather than
# from the substantive BJP-candidate coding itself. The existing pipeline flag
# is the quantity already used by all respondent analyses, so treat it as
# authoritative for BJP availability and retain the raw reconstruction as an
# explicit audit. Alliance-specific quantities are set to missing for conflicted
# PC-years rather than silently forcing a reconciliation.
if (
  ...
  warning(
    "Found ",
    mapping_audit$n_bjp_flag_mismatch,
    " AC-year BJP flag mismatches. ",
    "Proceeding with the existing pipeline BJP flag as authoritative; ",
    "raw NDA-alliance quantities will be suppressed in conflicted PC-years. ",
    "Review candidate_supply_bjp_mapping_mismatches.csv."
  )
}
...
ac_year_supply <- ac_year_supply |>
  dplyr::mutate(
    ...
    bjp_candidate_present_reconciled =
      dplyr::coalesce(
        bjp_candidate_present_existing,
        as.logical(
          bjp_candidate_present_derived
    ...
    bjp_candidate_flag_source =
      dplyr::case_when(
        !is.na(
          bjp_candidate_present_existing
        ) ~
          "Existing pipeline flag",

        !is.na(
          bjp_candidate_present_derived
        ) ~
          "Raw-election fallback",
...
pc_conflict_audit <- ac_year_supply |>
  ...
  dplyr::summarise(
    ...
    existing_flag_varies_within_pc =
      dplyr::n_distinct(
        bjp_candidate_present_reconciled[
          !is.na(
            bjp_candidate_present_reconciled
...
if (
  any(
    pc_conflict_audit$existing_flag_varies_within_pc
  )
) {
  stop(
    "The existing BJP candidate flag varies across AC segments within at least ",
    "one PC-year. This is a substantive data inconsistency and must be resolved."
  )
}
...
pc_year_context <- ac_year_supply |>
  ...
  dplyr::summarise(
    ...
    nda_candidate_present_raw =
      dplyr::first(
        nda_candidate_present
      ),

    nda_nominee_is_bjp_raw =
      dplyr::first(
        nda_nominee_is_bjp
      ),

    nda_ally_substitution_raw =
      dplyr::first(
        nda_ally_substitution
      ),
...
pc_year_context <- pc_year_context |>
  dplyr::mutate(
    nda_candidate_present =
      dplyr::if_else(
        candidate_mapping_conflict,
...
fit_always_contested <- function(
  ...
  tibble::tibble(
    ...
    interaction_order =
      order,
    sample =
      "BJP contested in both 2009 and 2014",
    interaction_estimate =
      beta,
    interaction_se =
      se,
...
always_contested_results <- dplyr::bind_rows(
  fit_always_contested(
    "muslim",
    "muslim_share_2001_dist_proxy",
    "two_way"
  ),
...
readr::write_lines(
  c(
    ...
    "-----------------------",
    "The existing pipeline BJP-candidate flag is authoritative for BJP availability because it is",
    "the flag used in the frozen respondent analyses. The independently reconstructed raw-election",
    "flag is retained as an audit. If they disagree after geography mapping, NDA-specific quantities",
    "are suppressed for that conflicted PC-year rather than silently forcing the raw mapping.",
    "",
    "HARD VALIDATION",
    "---------------",
    ...
    "3. Preferred respondent models restricted to PCs with BJP candidates in BOTH 2009 and 2014.",
    "4. Descriptive model-based decomposition:",
