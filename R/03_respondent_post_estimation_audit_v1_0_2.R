...
AUDIT_REVISION <-
  "2026-08-08-v1.0.2-frozen-reference-hotfix"
...
# Fail fast before plotting if a human-readable FDI-family label was lost
# during grouped summarization. This was the source of the v1.0 plotting error.
decomposition_label_checks <- list(
...
missing_fdi_label_objects <- names(
  decomposition_label_checks
)[
  !vapply(
    decomposition_label_checks,
    function(x) {
...
if (
  length(
    missing_fdi_label_objects
  ) > 0
) {
  stop(
    "Post-estimation summary lost fdi_family_label in: ",
    paste(
      missing_fdi_label_objects,
      collapse = ", "
    )
...
# Use the exact frozen substantive FDI movements stored in the completed
# specification universe. Do NOT recompute the median-positive exposure on
# the control-complete refit sample, because that would make the conditional
# plot use a different estimand from the saved headline contrast.
preferred_two_way_saved <-
  preferred_2014_muslim_rows |>
  dplyr::filter(
    interaction_order ==
      "two_way"
...
if (
  ...
  stop(
    "Could not uniquely resolve saved preferred two-way/triple reference rows."
  )
}

delta_fdi_two <-
  preferred_two_way_saved$delta_fdi[[1]]

delta_fdi_triple <-
  preferred_triple_saved$delta_fdi[[1]]
...
# Verify that the refitted coefficients reproduce the exact saved headline
# substantive contrasts at the frozen q25->q75 moderator movement.
two_saved_low <-
  preferred_two_way_saved$moderator_low[[1]]

two_saved_high <-
  preferred_two_way_saved$moderator_high[[1]]

tri_saved_low <-
  preferred_triple_saved$moderator_low[[1]]

tri_saved_high <-
  preferred_triple_saved$moderator_high[[1]]

two_low_effect <- linear_combo(
  preferred_two_way_fit,
  stats::setNames(
    c(
      delta_fdi_two,
...
preferred_refit_reproduction_check <-
  tibble::tibble(
    interaction_order =
      c(
        "two_way",
    ...
    saved_contrast_pp =
      c(
        preferred_two_way_saved$contrast_estimate[[1]],
        preferred_triple_saved$contrast_estimate[[1]]
      ),

    refit_reproduced_contrast_pp =
      c(
        two_refit_contrast_pp,
        tri_refit_contrast_pp
      )
...
readr::write_csv(
  ...
  file.path(
    audit_preferred_dir,
    "52_preferred_refit_reproduction_check.csv"
  )
)

if (
  any(
    !preferred_refit_reproduction_check$reproduces_saved
  )
) {
  stop(
    "Preferred-model refit failed to reproduce the frozen saved substantive contrast."
  )
}
