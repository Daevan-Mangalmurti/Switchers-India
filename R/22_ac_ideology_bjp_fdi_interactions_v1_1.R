# ============================================================
# 22_ac_ideology_bjp_fdi_interactions_v1_1.R
#
# Assembly-constituency models of BJP voting by ideological group.
# Revision: 2026-08-20-v1.1
#
# Core question:
...
#   7. estimates the 27-model design for each ideology (81 models total).
#
...
SCRIPT_REVISION <- "2026-08-20-v1.1-ideology-bjp-ac-fdi"
...
out_root <- file.path(
  paths$derived_dir,
  "model_exploration",
  "ac_ideology_bjp_fdi_interactions_v1_1"
)
...
# Backward-compatible name for the explicit 27 centrist formulas below.
...
fit_parallel_ideology_models <- function(group_prefix, data_2014, data_matched) {
  ...
  fit <- function(lhs, exposure, data, fe, baseline = NULL, year_filter = NULL) {
    ...
    fixest::feols(fml, data = fit_data, cluster = ~pc_cluster_id)
  }
...
saveRDS(models_81, file.path(out_data_dir, "models_all_ideologies_81.rds"))
...
readr::write_csv(
  model_audit,
  file.path(out_audit_dir, "03_model_sample_and_formula_audit_all_ideologies.csv")
)
...
interaction_comparison <- dplyr::bind_rows(
  purrr::imap_dfr(models_left_27, ~extract_focal_interaction(.x, "Left", .y)),
  purrr::imap_dfr(models_center_27, ~extract_focal_interaction(.x, "Center", .y)),
  purrr::imap_dfr(models_right_27, ~extract_focal_interaction(.x, "Right", .y))
)

interaction_comparison_wide <- interaction_comparison |>
  dplyr::select(ideology, model, estimate, std_error, p_value, nobs) |>
  tidyr::pivot_wider(
    names_from = ideology,
    values_from = c(estimate, std_error, p_value, nobs),
