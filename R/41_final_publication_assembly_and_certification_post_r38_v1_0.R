...
registry_path <-
  "config/paper_artifacts_v1_4.csv"
...
if (
  ...
  stop(
    "Missing authoritative post-R38 v1.4 paper artifact registry."
  )
}
...
output_root <-
  file.path(
    "outputs",
    "paper_outputs_final_post_r38_v1_0"
  )
...
alternate_registry_path <-
  "config/paper_artifact_alternates_v1_0.csv"
...
if (
  ...
  stop(
    "Missing retained post-R38 alternate-artifact registry."
  )
}
...
alternate_artifacts_exist <-
  nrow(
    alternate_registry
  ) > 0L &&
  all(
    alternate_registry$publication_status == "Built"
  ) &&
  all(
    file.exists(
      alternate_registry$publication_artifact
    )
  ) &&
  all(
    file.exists(
      alternate_registry$source_artifact
    )
  )

add_check(
  "C003A",
  "Every retained post-R38 alternate artifact and source exists",
  alternate_artifacts_exist,
  paste0(
    "Alternates=", nrow(alternate_registry),
    "; publication artifacts=", sum(file.exists(alternate_registry$publication_artifact)),
    "; sources=", sum(file.exists(alternate_registry$source_artifact))
  )
)
...
add_check(
  "C021",
  "Authoritative v1.4 paper registry does not reference R31d",
  !live_registry_mentions_r31d,
  paste0(
    "Live references=",
    as.integer(
...
add_check(
  "C022",
  "Authoritative v1.4 paper registry does not use stale R30 demographic registry",
  !live_registry_mentions_stale_r30_demog,
  paste0(
    "Live references=",
    as.integer(
...
scripts_to_parse <-
  c(
    ...
    "R/38c_voter_four_ideology_heterogeneity_v1_1.R",
    ...
    lapply(
      ...
      function(i) {
        ...
        tibble(
          ...
          source_path = row$publication_artifact,
...
config_files <-
  c(
    ...
    "config/paper_artifact_alternates_v1_0.csv",
    "config/paper_artifact_exclusions_v1_0.csv",
    "config/paper_display_decisions_v1_1.csv",
...
write_csv(
  ...
  file.path(
    cert_dir,
    "03b_alternate_publication_artifact_copy_registry.csv"
  )
)
...
readme <-
  c(
    ...
    "",
    "This directory is assembled from config/paper_artifacts_v1_4.csv.",
    "",
    "main/ contains the six registered main figures and two main tables.",
        "",
        "This post-R38 bundle is separate from outputs/paper_outputs_final_v1_0, which remains the historical R37/v1.3 bundle.",
    "",
    ...
    "",
    "No statistical model is estimated by R41.",
    "",
...
if (
  ...
  stop(
    "R41 final certification failed."
  )
}

cat(
  "\nR41_FINAL_PUBLICATION_CERTIFICATION_COMPLETE\n"
)
