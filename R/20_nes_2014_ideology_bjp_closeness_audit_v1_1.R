...
print(bjp_vote_by_ideology, n = Inf)
...
classification_transition <- audit %>%
  ...
  mutate(
    pct_of_3q_group_unweighted = 100 * n / sum(n),
    pct_of_3q_group_stpop_weighted =
      if_else(sum(sum_stpop) > 0, 100 * sum_stpop / sum(sum_stpop), NA_real_)
  ) %>%
  ungroup()
...
print(left_scheme_comparison, n = Inf)
