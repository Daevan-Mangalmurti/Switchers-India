...
item_audit_2009 <- map_dfr(
  ...
  function(item) {
    ...
    question_wording_this_item <- variable_label(raw09[[item]])
    ...
    full_codes |>
      ...
      transmute(
        item = .env$item,
        axis = spec$axis[[1]],
        question_wording = .env$question_wording_this_item,
        raw_code,
        response_label,
        oriented_score,
        ideology_bucket,
        audit_verdict = spec$audit_verdict[[1]],
        direction_rationale = spec$direction_rationale[[1]]
      )
  }
)
