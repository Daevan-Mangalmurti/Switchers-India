...
SYNTHESIS_REVISION <-
  "2026-08-09-v1.0.3-evidence-matrix-mask-hotfix"
...
# ac_change widens the NES Center-share measures but, by design, does not
# carry the underlying 2009 ideology-complete respondent denominator.
# Reconstruct that baseline reliability count exactly as the repaired v5.0
# aggregate specification script does.
ac_year_reference <- readRDS(
  file.path(
    paths$final_dir,
    "ac_year.rds"
  )
...
center_quality_2009 <- ac_year_reference |>
  dplyr::filter(
    year == 2009
  ) |>
  dplyr::select(
...
if (
  !"nes_n_ideology_complete_2009" %in%
    names(
      ac_change
    )
) {
...
center_count_reconstruction_audit <- tibble::tibble(
  n_ac_change =
    nrow(
      ac_change
    ),
  n_center_n2009_nonmissing =
    sum(
      !is.na(
        ac_change$nes_n_ideology_complete_2009
      )
    ),
  share_center_n2009_nonmissing =
    mean(
      !is.na(
        ac_change$nes_n_ideology_complete_2009
      )
...
readr::write_csv(
  center_count_reconstruction_audit,
  file.path(
    out_manifest_dir,
    "center_2009_denominator_reconstruction_audit.csv"
  )
)
