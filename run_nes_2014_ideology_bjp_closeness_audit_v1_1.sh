...
R_NAME="20_nes_2014_ideology_bjp_closeness_audit_v1_1.R"
...
# If the corrected R script is beside this runner in Downloads, install a copy
# into the project R directory. Otherwise use an already-installed project copy.
...
LOG="$PROJECT_ROOT/logs/nes_2014_ideology_bjp_closeness_audit_${STAMP}.log"

cd "$PROJECT_ROOT"

echo "Running corrected NES 2014 ideology audit..."
Rscript --vanilla "$PROJECT_R_SCRIPT" 2>&1 | tee "$LOG"

echo
echo "PASS"
echo "Log: $LOG"
echo "Outputs: $PROJECT_ROOT/outputs/nes_2014_ideology_audit_v1_1"
